...
def cargar_modelo():
    ...
    tfidf_vectorizer, svd_model, classifier_model = load(MODEL_FILE)
    return tfidf_vectorizer, svd_model, classifier_model


def svd(sintomas_usuario):
    """Carga el modelo guardado y devuelve la predicción con su probabilidad."""
    try:
        tfidf_vectorizer, svd_model, classifier_model = cargar_modelo()
    except FileNotFoundError as e:
        return f"Error: {e}", 0

    texto_limpio = preprocess_text(sintomas_usuario)
    vector_tfidf = tfidf_vectorizer.transform([texto_limpio])
    vector_svd = svd_model.transform(vector_tfidf)
